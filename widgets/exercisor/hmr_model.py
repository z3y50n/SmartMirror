...
def Encoder_fc3_dropout(
    x, num_output=85, is_training=True, reuse=False, name="3D_module"
):
    """
    ...
    with tf.compat.v1.variable_scope(name, reuse=reuse) as scope:
        ...
        small_xavier = variance_scaling_initializer(
            factor=0.01, mode="FAN_AVG", uniform=True
        )
...
class HMR(object):
    ...
    def build_test_model_ief(self):
        ...
        self.img_feat, self.E_var = img_enc_fn(
            self.images_pl, is_training=False, reuse=False
        )
        ...
        for i in np.arange(self.num_stage):
            ...
            if i == 0:
                delta_theta, _ = threed_enc_fn(
                    state, num_output=self.total_params, is_training=False, reuse=False
                )
            else:
                delta_theta, _ = threed_enc_fn(
