...
class GLMeshData(object):
    ...
    def __init__(self, vertices=None, faces=None, normals=None, indices=None, **kwargs):
    ...
    def normals(self, norms):
        if self.verts_formatted.shape[0] != norms.shape[0]:
            return (
                False,
                "The number of normals is not equal to the number of vertices",
            )
        self._normals = norms
        for i in range(self.verts_formatted.shape[0]):
    ...
    def texture_coords(self, text):
        if self.verts_formatted.shape[0] != text.shape[0]:
            return (
                False,
                "The number of texture coordinates is not equal to the number of vertices",
            )
