...
from .keypoint_editor import KeypointEditForm
...
class EditorControls(AbstractControls):
    ...
    def show_load_dialog(self):
        ...
        content = LoadDialog(
            save_from_video=self.predict_from_video, cancel=self.dismiss_popup
        )
    ...
    def predict_from_video(self, path, filename):
        ...
        self.actions["predict"].initialize(
            self.smpl_mode, source=source, save_exercise=True
        )
        self.frame_indx = 0
    ...
    def start_exercise(self, exercise_name: str):
        ...
        if exercise_name in self.exercises.keys():
            ...
            self.actions["playback"].initialize(
                self.smpl_mode, self.exercises[exercise_name]
            )

    def update(self, exercise_controller):
        super().update(exercise_controller)
        if self.actions["playback"].running:
            self.actions["playback"].exercise = self.exercises[
    ...
    def display_kpnt_edit_form(self, kpnt_name: str, keypoints_spec):
        ...
        try:
            ...
            self.kpnt_edit_form = KeypointEditForm(
                keypoints_spec=keypoints_spec,
                curr_kpnt=kpnt_name,
    ...
    def on_touch_down(self, touch):
        if self.actions["playback"].running and self.ids.prog_slider.collide_point(
            *touch.pos
        ):
