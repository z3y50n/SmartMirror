...
class Weather(Widget):
    ...
    _update_interval = ConfigParserProperty("", "weather", "update_interval", "Weather")
    ...
    def on_kv_post(self, dt):
        self.update_config()
        Clock.schedule_interval(self._get_weather, int(self._update_interval))
    ...
    def _get_weather(self, dt):
        try:
            ...
            self._icon = (
                f"http://openweathermap.org/img/w/{r['weather'][0]['icon']}.png"
            )
    ...
    def request_day(self, datetime="today"):
        ...
        return [
            ("speech", f"The weather {when} {desc} with {temperature} degrees Celcius")
        ]
    ...
    def request_location(self, location: str):
        ...
        return [("config", "weather", "city_name", location), ("speech", text)]
