...
class Renderer(Widget):
    ...
    def __init__(self, smpl_faces_path=None, keypoints_spec=None, obj_mesh_path=None):
        ...
        shader_path = os.path.join(os.path.abspath(os.path.dirname(__file__)), 'simple.glsl')
    ...
    def setup_scene(self, rendered_obj, opts: dict = {}):
        ...
        with self.canvas:
            ...
            self.rotx = Rotate(1, 1, 0, 0)  # so that the object does not break continuity
    ...
    def _create_arrow(self, start_point: np.array, dir_vec: np.array):
        ...
        shaft_data = GLMeshData(np.concatenate((start_point, end_point), axis=0),
                                normals=-np.ones((2, 3)), indices=[0, 1])
        arrow_shaft = Mesh(
        ...
        for j in range(npoints):
            points.append(0.03 * (bases[0] * math.sin(step * j) + bases[1] * math.cos(step * j)))
    ...
    def _highlight_keypoint(self):
        ...
        sphere_radius = self.keypoints_spec[self._highlighted_kpnt_indx]['hradius'] * self.scale.x
    ...
    def reset_highlight(self):
        ...
        self.canvas['sphere_color'] = (1., 1., 1.)
        self.canvas['sphere_radius'] = 0.
    ...
    def _kpnts_to_2D(self):
        ...
        for i, vert in enumerate(verts_3d):
            ...
            pos[:3] /= pos[3]
            pos[0] = (pos[0] + 1) * width / 2.
            pos[1] = (pos[1] + 1) * height / 2.
