...
class CircularProgressBar(ProgressBar):
    ...
    def draw(self):
        ...
        with self.canvas:
            ...
            Line(circle=(circle_center_x, circle_center_y, circle_radius), width=self._thickness)

            # Draw the progress line
            normalised = (self.value - self.min) / (self.max - self.min)
            if normalised > 0:
                Color(*self.progress_color)
                Line(circle=(circle_center_x, circle_center_y, circle_radius, 0, normalised * 360),
            ...
            Rectangle(texture=self._text_label.texture, size=self._label_size,
                      pos=(self.size[0] / 2 - self._label_size[0] / 2 + self.pos[0],
    ...
    def set_value(self, value):
        ...
        if self.mode == 'timer':
            self._text_label.text = f'{int(self.value / 60)}:{str(self.value % 60).zfill(2)}'
        else:
...
class ProgressCounter(FloatLayout):
    ...
    def on_counter(self, _, new_val):
        ...
        if (self.mode == 'repetition' and self.counter == self.max_count) \
                or (self.mode == 'timer' and self.counter == self.min_count):
            self._stop_action()
