...
class RotInputForm(BoxLayout):
    ...
    def __init__(self, type: str = 'fixed', group: int = 0, apply_rule: Callable = None,
                 cancel: Callable = None, **kwargs):
        self.type = type
        self.group = group
...
class KeypointEditForm(FloatLayout):
    ...
    def __init__(self, keypoints_spec, curr_kpnt, setup_highlight, exercise_controller, *args, **kwargs):
        self.keypoints_spec = keypoints_spec
        self._setup_highlight = setup_highlight
        self._exercise_controller = exercise_controller
        self._current_thetas = exercise_controller.exercises[exercise_controller.current_exercise]

        self.set_curr_keypoint(curr_kpnt)
        super().__init__(*args, **kwargs)

    def set_curr_keypoint(self, kpnt_name):
        self.kpnt_indx = next(i for i, kpnt in enumerate(self.keypoints_spec) if kpnt['name'] == kpnt_name)
    ...
    def kpnt_indx(self, new_indx):
        ...
        self.on_frame_indx(self, self.frame_indx)  # trigger update of the keypoint's rotation values

        # self._update_rules_form()

    def _update_rules_form(self):
        rules = self._exercise_controller.rules[self._exercise_controller.current_exercise]
    ...
    def on_frame_indx(self, instance, frame_indx):
        ...
        self.rotation = [round(rot * 180 / math.pi, 2) for rot in rot_thetas[3 * self._smpl_kpnt_indx: 3 * self._smpl_kpnt_indx + 3]]
        ...
        if checkbox.active:
            ...
            for child in children:
                if type(child) == RotInputForm and child.group == checkbox.group:
                    ...
                    new_checkbox = Factory.BackgroundCheckBox(group=checkbox.group, type=checkbox.type)
                    new_checkbox.bind(active=lambda chk, state: self.on_checkbox_active(chk))
    ...
    def _create_rot_input_form(self, type: str, axis: int, text: str = None):
        if text is None:
            text = str(self.rotation[axis]) \
                    if type == 'fixed' else '-90:90'

        hint_text = 'degrees' if type == 'fixed' else 'deg1:deg2'
        text_input = RotInputForm(type=type, group=str(axis), text=text, hint_text=hint_text,
                                  apply_rule=self._apply_rule, cancel=self.on_checkbox_active)
