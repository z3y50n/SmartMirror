...
class AbstractAction():
    ...
    @mainthread
    def render_mesh(self, renderer: Renderer = None, new_vertices: np.array = None, new_kpnts: np.array = None):
...
class PlaybackAction(AbstractAction):
    ...
    def seek(self, time_point: Union[int, float], fmt: Text = 'frame'):
        ...
        """
        if fmt == 'duration':
            time_point = self.smpl_thread.frame_index + self.smpl_thread.target_fps * time_point
...
class PredictAction(AbstractAction):
    ...
    def initialize(self, smpl_mode: Text, source: Text = None, save_exercise: bool = False):
    ...
    @mainthread
    def render_mesh(self, new_vertices: np.ndarray = None, new_kpnts: np.ndarray = None):
        """ Extends the :method: `render_mesh` of the :class: `AbstractAction`. """
        super().render_mesh(self.renderer, new_vertices, new_kpnts)
...
class PlayAction(AbstractAction):
    ...
    def __init__(self, threads: Dict, renderers: List[Renderer], *args, **kwargs):
        ...
        self.error_renderer.canvas['object_color'] = (1., 0., 0.)  # the error vectors default red color
        super().__init__(renderer)

        self._error_sample_win = 5  # the number of frames for the moving average of the error vectors
        self._smooth_sample_win = 3  # the number of frames for the HMR's smoothing average
        self._prev_kpnts = np.empty(shape=(0, 24, 3))

        self._progress_counter = ProgressCounter(stop_action=self.stop)  # the timer/counter widget
    ...
    def _start_new_rep(self):
        ...
        self._pred_frm_indx = 0  # the number of performed predictions for the current repetition
    ...
    @mainthread
    def render_correct_mesh(self, correct_verts: np.ndarray = None, correct_kpnts: np.ndarray = None,
                            frame: Dict = None):
    ...
    def _smooth_kpnts(self, curr_kpnts):
        ...
        verts = np.append(self._prev_kpnts, np.expand_dims(curr_kpnts, axis=0), axis=0).mean(axis=0, keepdims=True)
    ...
    def _render_error_vectors(self, predicted_kpnts, indx):
        ...
        """
        error_vectors = self._kpnt_err_vecs[indx - self._error_sample_win: indx,
                                            :, :].mean(axis=0)
